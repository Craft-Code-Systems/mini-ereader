...
| Ref | MPN | Footprint | Source | Notes |
|-----|-----|-----------|--------|-------|
| SW1-3,5,6 | Würth **WS-TASU 436351045816** (4.7×3.5mm side push, boss) | `ereader:WE_WS-TASU_436351045816` | ⚑ project lib (from WE datasheet) | **Side-actuated** — actuator faces the board edge (screen covers the front, magnets the back, so no top/bottom access). The old `SW_SPST_TL3342` was **top-actuated** — wrong. Land per WE datasheet rev 001.003: 4 pads 1.2×0.7mm at x=±2.8/y=±1.35 + **2× Ø0.75 boss holes** at y=±1.375 (pins 1≡3 / 2≡4). DRC vs datasheet before fab |
...
- `WE_WS-TASU_436351045816` — Würth WS-TASU side-push tactile (SW1-3, SW5, SW6); land + Ø0.75 boss holes drawn from the WE datasheet.
...
- **Assigned (import clean):** U1, U6, J1, J2 (full FH12-24S name), J3 (full FH12-6S name), J4, J5, L1/L2/LC, D1, **SW1-6** (SW1-3/5/6 → `ereader:WE_WS-TASU_436351045816`, SW4 → `ereader:ALPS_SLLB5_Lever`), all R/C.
